      ...
      {activeTab === 'LOGIN' && (
        <div className="max-w-md mx-auto p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-xs">
            ...
            {/* Quick Switch Persona Accounts for 1-Click Multi-User Testing */}
            <div className="pt-4 border-t border-slate-100 dark:border-slate-700/80 space-y-2.5">
              <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider text-center">
                ⚡ 1-Click Quick Sign In (User A & User B)
              </p>
              <div className="grid grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={async () => {
                    setLoginIdentifier('hari');
                    ...
                    try {
                      const res = await fetch('/api/auth/login', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ identifier: 'hari', password: 'India@123' }),
                      });
                      const data = await res.json();
                      if (data.success && data.user) {
                        setCurrentUser(data.user);
                  ...
                  className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 hover:bg-orange-50 dark:hover:bg-orange-950/40 text-left transition-colors cursor-pointer"
                >
                  <span className="text-[10px] font-bold text-orange-600 block">User A (Learner)</span>
                  <span className="font-extrabold text-xs text-slate-900 dark:text-white block">Hari Krishna</span>
                  <span className="text-[10px] text-slate-400">@hari • Sent Request</span>
                </button>

                <button
                  type="button"
                  onClick={async () => {
                    setLoginIdentifier('sai');
                    setLoginPassword('India@123');
                    setLoginLoading(true);
                    try {
                      const res = await fetch('/api/auth/login', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ identifier: 'sai', password: 'India@123' }),
                      });
                      const data = await res.json();
                      if (data.success && data.user) {
                        setCurrentUser(data.user);
                  ...
                  className="p-2.5 rounded-xl border-2 border-orange-400/80 bg-orange-50/40 dark:bg-orange-950/30 hover:bg-orange-100/50 text-left transition-colors cursor-pointer relative"
                >
                  <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                    <span>User B (Teacher)</span>
                    <span className="h-2 w-2 rounded-full bg-emerald-500 animate-ping" />
                  </span>
                  <span className="font-extrabold text-xs text-slate-900 dark:text-white block">saicharan</span>
                  <span className="text-[10px] text-slate-500 dark:text-slate-400">@sai • Receives Requests</span>
                </button>
              </div>
            </div>
          </form>
